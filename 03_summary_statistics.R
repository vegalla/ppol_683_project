...
dc <-
  st_read('project/_data/processed/dc_processed.shp')
...
dc %>%
  as_tibble() %>%
  group_by(year) %>%
  summarize(min_income = min(mdn_nc_, na.rm = TRUE),
...
dc %>%
  as_tibble() %>%
  group_by(year) %>%
  summarize(min_house = min(mdn_hs_, na.rm = TRUE),
...
dc %>%
  as_tibble() %>%
  group_by(year) %>%
  summarize(mean_single_father = mean(pct_sngl_f, na.rm = TRUE),
...
dc %>%
  as_tibble() %>%
  group_by(year) %>%
  summarize(mean_college_educated = mean(pct_bch, na.rm = TRUE))%>%
  ggplot(aes(x = year)) +
...
dc %>%
  as_tibble() %>%
  group_by(year) %>%
  summarize(mean_income = mean(mdn_nc_, na.rm = TRUE)) %>%
  mutate(rescale_income = rescale(mean_income, c(0,1)))
